...
//  TTS: Capacitor native plugin (Android fix)
// ─────────────────────────────────────────────
...
async function loadPhrases() {
  try {
    ...
    console.error("phrases.json error:", e);
  }
}
  ...
  sentences.forEach((s, i) => {
    ...
    card.querySelectorAll(".spk").forEach(b => {
      b.addEventListener("click", () => {
        const cls = b.classList.contains("en-s") ? "sp-en" : "sp-fr";
        speak(b.dataset.text, b, b.dataset.lc, cls);
        markHeard(i);
      });
    });
...
// ── TTS — Native Android via Capacitor ────────
function stopTTS() {
  ...
  if (currentBtn) {
    currentBtn.classList.remove("sp-en", "sp-fr");
    currentBtn = null;
  }
}
...
function showToast(msg) {
  ...
  if (!t) {
    ...
    t.id = "toast";
    t.style.cssText = "position:fixed;bottom:90px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:8px 18px;border-radius:20px;font-size:12px;z-index:99;transition:opacity .3s;";
